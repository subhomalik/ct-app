...
import { c as REQUESTS_IN_FLIGHT_LOADER_DOM_ID } from './index-fe5ba32e.esm.js';
...
const getPortalContainer = () => process.env.NODE_ENV === 'test' ? document.body : document.querySelector("#".concat(REQUESTS_IN_FLIGHT_LOADER_DOM_ID));
const RequestsInFlightLoader = () => {
  const hasRequestsInFlight = useSelector(state => Boolean(state.requestsInFlight && state.requestsInFlight.length > 0));
  if (!hasRequestsInFlight) return null;
  return /*#__PURE__*/ReactDOM.createPortal(jsx(LoadingSpinner, {
    children: jsx(FormattedMessage, _objectSpread({}, messages.labelLoading))
  }), getPortalContainer());
};
RequestsInFlightLoader.displayName = 'RequestsInFlightLoader';

export { RequestsInFlightLoader as default };
