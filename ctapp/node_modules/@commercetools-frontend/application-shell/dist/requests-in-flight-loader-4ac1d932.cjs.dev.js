...
const getPortalContainer = () => process.env.NODE_ENV === 'test' ? document.body : document.querySelector("#".concat(dist_commercetoolsFrontendApplicationShell.REQUESTS_IN_FLIGHT_LOADER_DOM_ID));
const RequestsInFlightLoader = () => {
  const hasRequestsInFlight = reactRedux.useSelector(state => Boolean(state.requestsInFlight && state.requestsInFlight.length > 0));
  if (!hasRequestsInFlight) return null;
  return /*#__PURE__*/ReactDOM__default["default"].createPortal(jsxRuntime.jsx(LoadingSpinner__default["default"], {
    children: jsxRuntime.jsx(reactIntl.FormattedMessage, _objectSpread({}, messages.labelLoading))
  }), getPortalContainer());
};
RequestsInFlightLoader.displayName = 'RequestsInFlightLoader';

exports["default"] = RequestsInFlightLoader;
