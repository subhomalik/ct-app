...
var _package = require('../../dist/package-3dc00543.cjs.dev.js');
...
async function run() {
  ...
  cli.command('start', usageStart).usage("\n\n  ".concat(usageStart)).alias('dev').action(async options => {
    ...
    const startCommand = shouldUseExperimentalBundler ? await Promise.resolve().then(function () { return require('../../dist/start-vite-1eace36f.cjs.dev.js'); }) : await Promise.resolve().then(function () { return require('../../dist/start-3b3737c3.cjs.dev.js'); });
  ...
  cli.command('build', usageBuild).usage("\n\n  ".concat(usageBuild)).option('--build-only', '(optional) If defined, the command only creates the production bundles without compiling the "index.html".', {
    ...
    const buildCommand = shouldUseExperimentalBundler ? await Promise.resolve().then(function () { return require('../../dist/build-vite-eaddf5a8.cjs.dev.js'); }) : await Promise.resolve().then(function () { return require('../../dist/build-1ebda1db.cjs.dev.js'); });
  ...
  cli.command('login', usageLogin).usage("\n\n  ".concat(usageLogin)).action(async options => {
    ...
    // Do this as the first thing so that any code reading it knows the right env.
    process.env.NODE_ENV = 'production';
    const loginCommand = await Promise.resolve().then(function () { return require('../../dist/login-4de7665c.cjs.dev.js'); });
  ...
  cli.command('config:sync', usageConfigSync).usage("\n\n  ".concat(usageConfigSync)).option('--dry-run', '(optional) Executes the command but does not send any mutation request.', {
    ...
    // Do this as the first thing so that any code reading it knows the right env.
    process.env.NODE_ENV = 'production';
    const configSyncCommand = await Promise.resolve().then(function () { return require('../../dist/config-sync-7bd9d3ed.cjs.dev.js'); });
