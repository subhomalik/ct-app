import { AdapterConfigurationStatus, AdapterInitializationStatus, AdapterSubscriptionStatus } from '@flopflip/types';
...
const AdapterContext = /*#__PURE__*/createContext(initialAdapterContext);
...
const selectAdapterConfigurationStatus = (adaptersStatus, adapterIdentifiers) => {
  const isReady = hasEveryAdapterStatus(AdapterConfigurationStatus.Configured, adaptersStatus, adapterIdentifiers);
  const isUnconfigured = hasEveryAdapterStatus(AdapterConfigurationStatus.Unconfigured, adaptersStatus, adapterIdentifiers);
  const isConfiguring = hasEveryAdapterStatus(AdapterConfigurationStatus.Configuring, adaptersStatus, adapterIdentifiers);
  const isConfigured = hasEveryAdapterStatus(AdapterConfigurationStatus.Configured, adaptersStatus, adapterIdentifiers);
  const status = {
    isReady,
    isUnconfigured,
    isConfiguring,
...
const isEmptyChildren = children => !isFunctionChildren(children) && Children.count(children) === 0;
...
const useAppliedAdapterArgsState = _ref => {
  ...
  const _useState = useState(initialAdapterArgs),
    _useState2 = _slicedToArray(_useState, 2),
    appliedAdapterArgs = _useState2[0],
    setAppliedAdapterArgs = _useState2[1];
  const applyAdapterArgs = useCallback(nextAdapterArgs => {
    /**
     * NOTE:
...
const useAdapterStateRef = () => {
  ...
  const setAdapterState = useCallback(nextAdapterState => {
    adapterStateRef.current = nextAdapterState;
  }, [adapterStateRef]);
  const getIsAdapterConfigured = useCallback(() => adapterStateRef.current === AdapterStates.CONFIGURED, [adapterStateRef]);
  const getDoesAdapterNeedInitialConfiguration = useCallback(() => adapterStateRef.current !== AdapterStates.CONFIGURED && adapterStateRef.current !== AdapterStates.CONFIGURING, [adapterStateRef]);
...
const usePendingAdapterArgsRef = appliedAdapterArgs => {
  const pendingAdapterArgsRef = useRef(undefined);
  const setPendingAdapterArgs = useCallback(nextReconfiguration => {
...
const usePendingAdapterArgsEffect = _ref5 => {
   ...
   */
  const reconfigureOrQueue = useCallback((nextAdapterArgs, options) => {
    if (getIsAdapterConfigured()) {
...
function ConfigureAdapter(props) {
  ...
  return /*#__PURE__*/React.createElement(AdapterContext.Provider, {
    ...
    const isAdapterConfigured = props.adapter.getIsConfigurationStatus(AdapterConfigurationStatus.Configured);
...
const getNormalizedFlagName = flagName => camelCase(flagName);
...
function useAdapterSubscription(adapter) {
   ...
   *    is a singleton).
   */
  const useAdapterSubscriptionStatusRef = useRef(AdapterSubscriptionStatus.Subscribed);
  ...
  return useCallback(demandedAdapterSubscriptionStatus => useAdapterSubscriptionStatusRef.current === demandedAdapterSubscriptionStatus, [useAdapterSubscriptionStatusRef]);
}
